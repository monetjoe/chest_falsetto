        ...
        if response.status_code == 200:
            ...
            if len(cname_tds) > 0:
                for cname_td in cname_tds:
                    cname.append(cname_td.find_next(
                        'span', lang='zh-Hans').text)
        ...
        if response.status_code == 200:
            ...
            for subarea_a in tqdm(subarea_as, desc='Updating subareas...'):
                ...
                while (not cname) and (not tags):
                    print(f'Failed to get [{subarea_name}] info, retrying...')
                    rand_sleep(1, 2)
                    cname, tags = get_subarea_info(subarea_url)
            ...
            if nextpage:
                ...
                nextpage_subareas = get_subareas(nextpage_url)
                while not nextpage_subareas:
                    print(
                        f'Failed to load next page [{nextpage_url}], retrying...')
                    rand_sleep(1, 2)
                    nextpage_subareas = get_subareas(nextpage_url)

                subareas = merge_dicts(subareas, nextpage_subareas)
...
def save_subareas(subareas_path='./data/subareas.json', force_upd=True):
    if force_upd or ((not force_upd) and (not os.path.exists(subareas_path))):
        subarea_dict = get_subareas()
        with open(subareas_path, 'w', encoding='utf-8') as json_file:
            json.dump(subarea_dict, json_file, ensure_ascii=False, indent=4)

        print(f'Subareas have been updated into {subareas_path}')
        print(f'{len(subarea_dict.keys())} in total')
